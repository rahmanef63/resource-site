# Primitive — public-profile

> **Tier:** L
> **Lifted from:** CareerPack/frontend/app/[slug]/page.tsx + convex/profile/queries.ts (getPublicBySlug)

## Tujuan

SSR public profile page at /[slug] — Convex query, OG meta, JSON-LD, no auth.
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const profile = await fetchQuery(api.profile.queries.getPublicBySlug, { slug });
  if (!profile) notFound();
