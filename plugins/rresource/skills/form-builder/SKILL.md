---
description: zod schema → react-hook-form bindings → shadcn Form components. Generic factory. Trigger when user runs /rresource:form-builder or asks to add the "form-builder" primitive to the current project.
---

# rresource:form-builder

**Kind:** primitive
**Bundled spec:** `cookbook/primitives/form-builder.md`
**Shared protocol:** `SHARED.md` (read once per project).

## Run protocol

1. **Verify baseline.** Read `SHARED.md` §1. STOP if any row missing.
2. **Read bundled spec.** Open `cookbook/primitives/form-builder.md`.
