---
description: Drag-drop status columns w/ @dnd-kit/core. Generic items, generic columns, persists order via on-change callback. Trigger when user runs /rresource:kanban-board or asks to add the "kanban-board" primitive to the current project.
---

# rresource:kanban-board

**Kind:** primitive
**Bundled spec:** `cookbook/primitives/kanban-board.md`
**Shared protocol:** `SHARED.md` (read once per project).

## Run protocol

1. **Verify baseline.** Read `SHARED.md` §1. STOP if any row missing.
2. **Read bundled spec.** Open `cookbook/primitives/kanban-board.md`.
