---
description: Textarea + live preview tab w/ slash commands + markdown shortcuts. No contenteditable — keeps SSR clean. Trigger when user runs /rresource:markdown-editor or asks to add the "markdown-editor" primitive to the current project.
---

# rresource:markdown-editor

**Kind:** primitive
**Bundled spec:** `cookbook/primitives/markdown-editor.md`
**Shared protocol:** `SHARED.md` (read once per project).

## Run protocol

1. **Verify baseline.** Read `SHARED.md` §1. STOP if any row missing.
2. **Read bundled spec.** Open `cookbook/primitives/markdown-editor.md`.
