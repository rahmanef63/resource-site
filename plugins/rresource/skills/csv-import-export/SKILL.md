---
description: Papa Parse import w/ schema validation + JSON-to-CSV download. Streaming for large files. Trigger when user runs /rresource:csv-import-export or asks to add the "csv-import-export" primitive to the current project.
---

# rresource:csv-import-export

**Kind:** primitive
**Bundled spec:** `cookbook/primitives/csv-import-export.md`
**Shared protocol:** `SHARED.md` (read once per project).

## Run protocol

1. **Verify baseline.** Read `SHARED.md` §1. STOP if any row missing.
2. **Read bundled spec.** Open `cookbook/primitives/csv-import-export.md`.
