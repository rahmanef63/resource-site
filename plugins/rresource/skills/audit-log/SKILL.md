---
description: User-action audit trail. Append-only Convex table + admin viewer w/ filter. Trigger when user runs /rresource:audit-log or asks to add the "audit-log" primitive to the current project.
---

# rresource:audit-log

**Kind:** primitive
**Bundled spec:** `cookbook/primitives/audit-log.md`
**Shared protocol:** `SHARED.md` (read once per project).

## Run protocol

1. **Verify baseline.** Read `SHARED.md` §1. STOP if any row missing.
2. **Read bundled spec.** Open `cookbook/primitives/audit-log.md`.
