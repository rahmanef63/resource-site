---
description: Convert any DOM subtree to printable PDF client-side. No server roundtrip. ATS-safe text-selectable PDF, not raster. Trigger when user runs /rresource:pdf-generator or asks to add the "pdf-generator" primitive to the current project.
---

# rresource:pdf-generator

**Kind:** primitive
**Bundled spec:** `cookbook/primitives/pdf-generator.md`
**Shared protocol:** `SHARED.md` (read once per project).

## Run protocol

1. **Verify baseline.** Read `SHARED.md` §1. STOP if any row missing.
2. **Read bundled spec.** Open `cookbook/primitives/pdf-generator.md`.
