---
description: Resend wrapper w/ template registry + retry + rate-limit + spam-check. Server-only. Trigger when user runs /rresource:email-pipeline or asks to add the "email-pipeline" primitive to the current project.
---

# rresource:email-pipeline

**Kind:** primitive
**Bundled spec:** `cookbook/primitives/email-pipeline.md`
**Shared protocol:** `SHARED.md` (read once per project).

## Run protocol

1. **Verify baseline.** Read `SHARED.md` §1. STOP if any row missing.
2. **Read bundled spec.** Open `cookbook/primitives/email-pipeline.md`.
