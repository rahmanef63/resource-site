---
description: recharts wrapper w/ Convex aggregation queries — line/bar/pie/area + range picker + export PNG. Trigger when user runs /rresource:analytics-dashboard or asks to add the "analytics-dashboard" primitive to the current project.
---

# rresource:analytics-dashboard

**Kind:** primitive
**Bundled spec:** `cookbook/primitives/analytics-dashboard.md`
**Shared protocol:** `SHARED.md` (read once per project).

## Run protocol

1. **Verify baseline.** Read `SHARED.md` §1. STOP if any row missing.
2. **Read bundled spec.** Open `cookbook/primitives/analytics-dashboard.md`.
