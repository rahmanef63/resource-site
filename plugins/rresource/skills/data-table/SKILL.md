---
description: TanStack Table v8 wrapper w/ filter/sort/pagination/CSV export. Headless, schema-driven, ResponsiveDialog row inspector. Trigger when user runs /rresource:data-table or asks to add the "data-table" primitive to the current project.
---

# rresource:data-table

**Kind:** primitive
**Bundled spec:** `cookbook/primitives/data-table.md`
**Shared protocol:** `SHARED.md` (read once per project).

## Run protocol

1. **Verify baseline.** Read `SHARED.md` §1. STOP if any row missing.
2. **Read bundled spec.** Open `cookbook/primitives/data-table.md`.
