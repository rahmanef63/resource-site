---
description: Full auth platform (XL). LoginPage + forgot/reset flow + @convex-dev/auth Password+Anonymous + custom PBKDF2-SHA256 100k + JWT keys. Trigger when user runs /rresource:auth or asks to scaffold "auth" template into the current project.
---

# rresource:auth

**Kind:** feature
**Bundled spec:** `cookbook/features/auth.md`
...
2. **Read bundled spec.** Open `cookbook/features/auth.md` (relative to plugin
