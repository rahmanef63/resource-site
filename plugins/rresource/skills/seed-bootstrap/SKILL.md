---
description: Per-user starter data on first login. Idempotent. Run inside login flow after signUp. Trigger when user runs /rresource:seed-bootstrap or asks to add the "seed-bootstrap" primitive to the current project.
---

# rresource:seed-bootstrap

**Kind:** primitive
**Bundled spec:** `cookbook/primitives/seed-bootstrap.md`
**Shared protocol:** `SHARED.md` (read once per project).

## Run protocol

1. **Verify baseline.** Read `SHARED.md` §1. STOP if any row missing.
2. **Read bundled spec.** Open `cookbook/primitives/seed-bootstrap.md`.
