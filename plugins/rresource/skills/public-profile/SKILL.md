---
description: SSR public profile page at /[slug] — Convex query, OG meta, JSON-LD, no auth. Trigger when user runs /rresource:public-profile or asks to add the "public-profile" primitive to the current project.
---

# rresource:public-profile

**Kind:** primitive
**Bundled spec:** `cookbook/primitives/public-profile.md`
**Shared protocol:** `SHARED.md` (read once per project).

## Run protocol

1. **Verify baseline.** Read `SHARED.md` §1. STOP if any row missing.
2. **Read bundled spec.** Open `cookbook/primitives/public-profile.md`.
