---
description: Language toggle + per-route locale resolution. Server-component-friendly. localStorage + cookie persist. Trigger when user runs /rresource:i18n-switcher or asks to add the "i18n-switcher" primitive to the current project.
---

# rresource:i18n-switcher

**Kind:** primitive
**Bundled spec:** `cookbook/primitives/i18n-switcher.md`
**Shared protocol:** `SHARED.md` (read once per project).

## Run protocol

1. **Verify baseline.** Read `SHARED.md` §1. STOP if any row missing.
2. **Read bundled spec.** Open `cookbook/primitives/i18n-switcher.md`.
