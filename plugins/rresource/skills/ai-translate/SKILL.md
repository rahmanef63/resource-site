---
description: Generic LLM-powered text translation pipeline. Source-language-agnostic. Honors quota + sanitize + wrapUserInput. Trigger when user runs /rresource:ai-translate or asks to add the "ai-translate" primitive to the current project.
---

# rresource:ai-translate

**Kind:** primitive
**Bundled spec:** `cookbook/primitives/ai-translate.md`
**Shared protocol:** `SHARED.md` (read once per project).

## Run protocol

1. **Verify baseline.** Read `SHARED.md` §1. STOP if any row missing.
2. **Read bundled spec.** Open `cookbook/primitives/ai-translate.md`.
