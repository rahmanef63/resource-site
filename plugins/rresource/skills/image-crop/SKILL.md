---
description: react-easy-crop wrapper w/ dialog + aspect ratio presets + WebP output. Composes w/ file-upload pipeline. Trigger when user runs /rresource:image-crop or asks to add the "image-crop" primitive to the current project.
---

# rresource:image-crop

**Kind:** primitive
**Bundled spec:** `cookbook/primitives/image-crop.md`
**Shared protocol:** `SHARED.md` (read once per project).

## Run protocol

1. **Verify baseline.** Read `SHARED.md` §1. STOP if any row missing.
2. **Read bundled spec.** Open `cookbook/primitives/image-crop.md`.
