---
description: E-commerce / blog public storefront — Convex read-only + cart + currency + i18n. Trigger when user runs /rresource:cms-storefront or asks to scaffold "cms-storefront" template into the current project.
---

# rresource:cms-storefront

**Kind:** layout
**Bundled spec:** `cookbook/layouts/cms-storefront.md`
...
2. **Read bundled spec.** Open `cookbook/layouts/cms-storefront.md` (relative to plugin
...
If a configurable variant of this template exists on the kitab site
(`https://resource.rahmanef.com`), the user can:

1. Visit the template detail page.
2. Open the Inspector (top-right of FeatureBar).
3. Pick variant + addons. Live preview reacts.
4. Click **Copy prompt** — copies a dynamic agent prompt with the
   exact selections embedded.
5. Click **View source** — opens the GitHub source folder.

When invoked from the user's clipboard, the prompt already contains
the chosen variant + addon flags. Read it verbatim and act on it.
For the bundled-multiple flow, see `/build` on the site.
