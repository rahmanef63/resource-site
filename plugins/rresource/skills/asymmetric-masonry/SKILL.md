---
description: 8-slot repeating asymmetric grid w/ IntersectionObserver staggered scroll-reveal. Trigger when user runs /rresource:asymmetric-masonry or asks to scaffold "asymmetric-masonry" template into the current project.
---

# rresource:asymmetric-masonry

**Kind:** layout
**Bundled spec:** `cookbook/layouts/asymmetric-masonry.md`
...
2. **Read bundled spec.** Open `cookbook/layouts/asymmetric-masonry.md` (relative to plugin
...
If you can reach the kitab site (`https://resource.rahmanef.com`),
this template ships a configurable assembler at its detail page.
User picks variant + addons → live preview reacts → "Copy prompt"
emits a dynamic agent prompt with selections already embedded.

When the user pastes such a prompt, read it verbatim — the variant +
addon flags are spelled out at the top. Apply exactly.

For multi-template bundles see `/build` on the site.
