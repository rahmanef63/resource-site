---
description: GitHub + Google OAuth providers for @convex-dev/auth. Drop-in next to Password provider. Trigger when user runs /rresource:oauth-providers or asks to add the "oauth-providers" primitive to the current project.
---

# rresource:oauth-providers

**Kind:** primitive
**Bundled spec:** `cookbook/primitives/oauth-providers.md`
**Shared protocol:** `SHARED.md` (read once per project).

## Run protocol

1. **Verify baseline.** Read `SHARED.md` §1. STOP if any row missing.
2. **Read bundled spec.** Open `cookbook/primitives/oauth-providers.md`.
