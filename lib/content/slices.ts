...
export default convexAuthNextjsMiddleware();`,
  ...
  },
  {
    ...
    title: "Block Selection — multi-select + bulk delete",
    category: "ui",
    kind: "ui",
    version: "0.1.0",
    tagline: "Framework-agnostic multi-select for any list. Edge-click, Shift-range, Cmd-toggle, bulk delete.",
    description: "Framework-agnostic multi-selection for any vertical list (Notion-style blocks, table rows, cards). BlockSelectionProvider owns the selected-id set; wrap each item in SelectableBlock — thin top/bottom edge strips select on click (Shift = range from the anchor, Cmd/Ctrl = toggle). Backspace/Delete bulk-deletes when focus is outside an editable, Escape clears, and a floating count toolbar offers Delete/Clear. Pure UI — the host owns the data and runs onBulkDelete(ids). Pairs with notion-shell (the notion-clone template wires it onto the editor); lifted from notion-page-clone's block-selection slice.",
    source: "notion-page-clone",
    slicePath: "frontend/slices/block-selection",
    convexPaths: [],
    npm: [],
    shadcn: [],
    env: [],
