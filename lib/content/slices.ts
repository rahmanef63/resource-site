...
export default convexAuthNextjsMiddleware();`,
  ...
  },
  {
    ...
    kind: "full",
    version: "0.2.0",
    tagline: "Members + invites — table · inline role change · InviteDialog · pending list. Props-driven, RBAC-agnostic.",
    description: "Members + invites surface ported from superspace, props-driven + RBAC-agnostic. <MembersPanel>: searchable / role-filterable / sortable member table (avatar + name + email), inline role dropdown, soft-remove, an InviteDialog (email + role + message) and a PendingInvites list with resend / cancel — all permission-gated. You pass `roles` + `currentPerms` (resolved from rbac-roles at the app level) + CRUD/invite callbacks; the slice imports no other slice's frontend. Convex template ships um_members + um_invites + listMembers / listInvites + addMember / updateMemberRole / removeMember + sendInvite / cancelInvite / resendInvite / acceptInvite (7-day token), all gated via rbac-roles' requirePermission. P1+P2 of the user-management epic — roles admin (P3) next.",
    source: "superspace",
