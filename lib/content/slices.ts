...
export default convexAuthNextjsMiddleware();`,
  ...
  },
  {
    ...
    kind: "full",
    version: "0.4.0",
    tagline: "Members + invites + roles + teams — tabbed, permission-gated. Props-driven, RBAC-agnostic.",
    description: "Full members + invites + roles-admin + teams surface ported from superspace, props-driven + RBAC-agnostic. <UserManagementPanel> tabs Members + Roles + Teams: member table (search / filter / sort, inline role dropdown, soft-remove), InviteDialog + PendingInvites (resend / cancel), a RolesPanel to create / edit / delete custom roles via a permission matrix (system roles read-only), and a TeamsPanel (named user groups + add / remove members). All permission-gated. You pass `roles` + `currentPerms` + the permission catalog (resolved from rbac-roles at the app level) + callbacks; the slice imports no other slice's frontend. Convex template ships um_members + um_invites + um_teams + um_team_members + member / invite / team endpoints (gated via rbac-roles' requirePermission); roles CRUD reuses rbac-roles' listRoles / upsertRole / removeRole. P1–P4a of the user-management epic — hierarchy + access matrix (P4b/c) next.",
    source: "superspace",
