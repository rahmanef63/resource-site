...
export default function FileExplorerDemo() {
  ...
  {
    slug: "glass-desktop",
    title: "Lucent Desktop",
    category: "os",
    kind: "ui",
    version: "0.2.0",
    tagline: "A configurable two-space glass desktop with a 47-widget catalog, persisted layout engine, picker, and gallery.",
    description:
      "Interactive dark-glass desktop with two spaces and a catalog-driven widget canvas. The React/Next default keeps its specialized widget renderers; the native Svelte 5/SvelteKit distribution uses the same 47-widget catalog, default seed, grid packer, layout mutations, and LayoutStore persistence contract with a native data-driven renderer. Both frameworks support add/remove/resize/move, edit mode, space switching, reset, persistence, and a full widget gallery. Consumer branding and storage are injected through props; no project-specific URLs, roles, or credentials are embedded.",
    source: "rahmanef63/resource-site",
    slicePath: "frontend/slices/glass-desktop",
    convexPaths: [],
    npm: ["lucide-react@^0.400.0", "sonner@^2.0.7", "clsx@^2.1.1", "tailwind-merge@^3.5.0"],
    shadcn: ["button", "checkbox", "dialog", "dropdown-menu", "input", "scroll-area", "slider", "toggle"],
    env: [],
    peers: [],
    tags: ["shell", "desktop", "widgets", "glass", "os", "layout", "local-storage"],
    resourceType: "module",
    maturity: "stable",
    previewPath: "/preview/slices/glass-desktop",
    defaultView: "desktop",
    defaultZoom: 0.55,
    install: "npx rr add glass-desktop",
    agentRecipe: "Run `npx rr add glass-desktop` for React/Next or `npx rr add glass-desktop --framework sveltekit` for native Svelte 5. Mount <GlassDesktop /> full-bleed, optionally pass brand/initialSpace and a custom LayoutStore; use gallery mode to inspect the full 47-widget catalog.",
  },
  {
    slug: "appshell",
...
export default async function Page({ params }: { params: Promise<{ slug?: string[] }> }) {
  ...
  },
  {
    ...
    peers: [],
    tags: ["auth", "convex", "password", "magic-link", "google", "anonymous", "no-clerk", "pbkdf2", "svelte", "framework-parity"],
    usedBy: ["personal-brand-os", "wirausaha-os", "konsultan-os"],
    agentRecipe: "Run `rr add convex-auth` for React/Next or `rr add convex-auth --framework sveltekit` for native Svelte. React uses @convex-dev/auth/react. Svelte SignInPage requires a host-supplied AuthFlow because the upstream package has no official Svelte adapter. Keep JWT/OAuth/Resend env server-side.",
...
// internal.actions.send.sendCampaign → scheduled internal Resend worker`,
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    description: "Framework-parity AI workspace with chat, studio, and agents variants. React/Next remains default; native Svelte 5/SvelteKit mirrors all three over shared chat/generation/runner/tool cores. Variant installs are exact: only chat pulls convex/features/aiChat + ai + @ai-sdk/anthropic + ANTHROPIC_API_KEY; studio and agents remain frontend-only and provider-free.",
    source: "rahmanef63/resource-site",
    docsUrl: "https://sdk.vercel.ai/docs",
    install: "npx rr add ai-workspace <chat|studio|agents> [--framework sveltekit]",
    ...
    env: [],
    peers: [],
    tags: ["ai", "ai:chatbot", "ai:studio", "ai:agent", "agent-mode", "tool-calls", "streaming", "generation", "async", "traces", "multimodal", "svelte", "framework-parity", "variant-gated"],
    usedBy: [],
    variants: [
...
<ResponsiveDialog open={open} onOpenChange={setOpen} variant="modal" size="md">
  ...
  },
  {
    ...
    convexPaths: [],
    npm: ["lucide-react@^0.400.0", "vaul@^1.0.0"],
    shadcn: ["sidebar", "drawer", "button", "separator", "sheet", "tooltip"],
    env: [],
    peers: [],
...
// BroadcastChannel is preferred; same-origin storage events are the fallback.`,
  ...
  },
  {
    ...
    description: "Framework-parity user administration for members, hierarchy-aware invites, custom roles, teams, and the cross-tenant access matrix. React/Next remains the default shadcn renderer; native Svelte 5/SvelteKit reuses the same framework-neutral types, RBAC checks, members query/sort core, agent tools, and Convex backend. Authorization stays server-side in the bundled permission-gated Convex functions.",
    source: "superspace",
...
// Register eventTrackingTools with analytics in your tool host.`,
  ...
  },
  {
    ...
    usedBy: [],
    agentRecipe: "React/default: `npx rr add notion-app` (legacy alias `notion` remains). SvelteKit: `npx rr add notion-app --framework sveltekit`. Both use the same EditorDataAdapter page/block CRUD contract and convex/features/notion helpers. React mounts `<EditorAdapterProvider><PageEditor/></EditorAdapterProvider>` for the full rich capability surface; Svelte mounts `<PageEditor pageId data />` and accepts optional navigation/tool registration. Keep authz in the host/backend adapter.",
  },
  {
    slug: "sections",
  ...
  },
  {
    ...
    version: "0.25.0",
    maturity: "beta",
    tagline: "Pure Notion-clone primitives in 3 variants — page editor · 11-view database · tree sidebar — with React/Next default plus native SvelteKit.",
    description: "Framework-parity, props-driven Notion primitives over one shared domain model. React/Next remains default; native Svelte 5/SvelteKit now provides page, database and sidebar variants. Page keeps block CRUD, slash/markdown transforms and native drag reorder; database renders all 11 view kinds over the same filter/sort/formula/aggregate/CSV/JSON engines; sidebar reuses the same tree projection helpers with native drag/reparent. Host owns all data and authorization.",
    source: "notion-page-clone",
    slicePath: "frontend/slices/notion-ui",
    convexPaths: [],
    npm: ["@dnd-kit/core@^6.3.1", "@dnd-kit/sortable@^10.0.0", "@dnd-kit/utilities@^3.2.2", "katex@^0.16.47", "highlight.js@^11.11.1", "recharts@^3.8.0", "lucide-react@^1.16.0"],
    ...
    peers: [],
    tags: ["ui", "notion", "notion-like", "database", "table", "board", "list", "gallery", "calendar", "feed", "chart", "dashboard", "form", "map", "timeline", "gantt", "kanban", "views", "filter", "sort", "property", "files", "person", "formula", "timestamp", "unique-id", "csv", "json", "import", "export", "template", "data", "backup", "primitive", "optional", "embeddable", "svelte", "framework-parity"],
    usedBy: ["notion-page-clone-os"],
    agentRecipe: "React/default: `npx rr add notion-ui <page|database|sidebar>`; append `--framework sveltekit` for native Svelte 5. Omit the variant to install all three. Both frameworks share the same Block/Page/Property/Database domain model and pure database/tree engines; data stays host-owned through callbacks. Svelte database renders table, board, list, gallery, calendar, feed, chart, dashboard, form, map and timeline without React/dnd-kit/Recharts/Lucide/shadcn runtime.",
