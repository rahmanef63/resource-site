...
// Any tab on the same origin sees the change instantly.`,
  ...
  },
  {
    ...
    kind: "full",
    version: "0.1.0",
    tagline: "The ideal admin panel: a gated shell + 26-section registry that mounts your existing slices, plus 5 net-new gap sections.",
    description: "The ideal admin panel as a portable slice, distilled from ~15 project admin panels (docs/admin-panel/COMPARISON.md + DESIGN.md). A gated two-column shell (AdminConsole) over a 26-section catalog (ADMIN_CONSOLE_SECTIONS) spanning observability / identity / ai / content / commerce / config — each section tagged with the rr slice that supplies it or `self` for the gap sections. Reuse sections mount from a consumer-supplied `components` map (no slice→slice imports); the 5 owned gap sections nothing else covered — Analytics, Audit-log viewer, Navigation config, SEO health, Leads/CRM inbox — are adapter-driven with in-memory mocks so the whole console runs backend-free. Access is injected (no hard Convex import); gate logic is pure + unit-tested. Convex copy-source ships ac_leads + ac_nav_items with requireAdmin-gated CRUD.",
    source: "rahmanef63/resource-site",
    slicePath: "frontend/slices/admin-console",
    convexPaths: ["convex/features/admin_console"],
    npm: [],
    shadcn: ["card", "button", "badge", "input", "table", "select", "sheet", "scroll-area", "progress", "textarea"],
    env: [{ name: "PLATFORM_ADMIN_EMAILS", scope: "convex", required: false, description: "Comma-separated superadmin email allowlist; resolves the platform_admin access level." }],
    peers: [
      { slug: "convex-auth", range: "^0.1", reason: "requireAdmin gate on ac_leads / ac_nav_items uses convex-auth identity." },
      { slug: "rbac-roles", range: "^0.1", reason: "section gates are permission tokens resolved by rbac-roles." },
    ],
    tags: ["infra", "admin", "console", "dashboard", "composition", "rbac", "crud", "registry"],
    usedBy: [],
    agentRecipe: "Run `rr add admin-console`. Mount <AdminConsole access={useAdminAccess()} components={{ users: <UsersPanel/>, ... }} /> at /admin — owned sections (analytics, audit-log, nav-config, seo-health, leads) render as-is; map each reuse section id to the panel from the peer slice you installed. Compose adminConsoleTables into convex/schema.ts and front ac_leads.create with the rate-limit slice.",
    previewPath: "/preview/slices/admin-console",
    defaultView: "desktop",
    defaultZoom: 0.6,
  },
  {
  ...
  },
  {
    ...
    kind: "ui",
    version: "0.4.0",
    tagline: "Marketing/landing-page composition: admin CRUD + config-driven public section renderers (stats/testimonials/pricing/faq/newsletter/custom). Absorbs the old feature-grid/faq-section/pricing-page/testimonials-grid/blog-section/portfolio-section/changelog-feed slugs.",
    ...
    convexPaths: [],
    npm: ["lucide-react@^0.400.0", "embla-carousel-autoplay@^8.6.0", "next@^15"],
  ...
  },
  {
    ...
    kind: "ui",
    version: "0.1.0",
    tagline: "Tasteful entrance animations with no motion library: Reveal/Stagger/CountUp/Marquee + embla carousel + radix accordion. Reduced-motion safe.",
