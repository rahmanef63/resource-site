...
export default convexAuthNextjsMiddleware();`,
  ...
  },
  {
    ...
    kind: "full",
    version: "0.1.0",
    tagline: "Members table — search · filter · inline role change · invite. Props-driven, RBAC-agnostic.",
    description: "Members surface ported from superspace, props-driven + RBAC-agnostic. <MembersPanel>: searchable / role-filterable / sortable member table with avatar + name + email, inline role dropdown, soft-remove, and a permission-gated invite button. You pass `roles` (options) + `currentPerms` (resolved permission strings, wired from rbac-roles at the app level) + CRUD callbacks; the slice imports no other slice's frontend. Convex template ships um_members + listMembers (joins users) + addMember / updateMemberRole / removeMember, all gated via rbac-roles' requirePermission. P1 of the user-management epic — invites (P2) + roles admin (P3) next.",
    source: "superspace",
