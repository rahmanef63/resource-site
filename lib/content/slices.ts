...
export default convexAuthNextjsMiddleware();`,
  ...
  {
    slug: "notion-shell",
    title: "Notion Shell — page/sidebar/database wrappers + slash menu + decorator + 6 views",
    category: "ui",
    kind: "ui",
    version: "0.3.0",
    description: "Portable Notion-style wrapper primitives. PAGE EDITOR: NotionPage / NotionHeader / NotionSidebar / NotionBlock (live inline-markdown decorator, hover actions menu), SlashMenu (searchable block-type picker w/ keyboard nav), BlockActionsMenu (turn-into / duplicate / delete), InsertBlockButton (`+` trigger w/ SlashMenu). DATABASE: NotionDatabase (full DB surface w/ tabs + options + per-column menu), NotionProperty (10 property-cell types: text/number/checkbox/select/multi-select/status/date/url/email/phone), 6 built-in views (Table/Board/List/Gallery/Calendar/Feed), ViewTabs, ViewOptions (sort + filter + search popover), ColumnHeaderMenu (rename / change type / sort / hide / delete). Pure helpers: applyView, groupBy (board kanban), bucketByDate (calendar grid). Pure / props-driven — host owns data + change handlers. Drop into any React surface; pair with notion-blocks for the four editor primitives.",
    source: "notion-page-clone",
    ...
    env: [],
    peers: [],
    tags: ["ui", "notion", "shell", "wrapper", "sidebar", "page", "database", "primitive", "portable", "slash-menu", "decorator", "wysiwyg", "views", "kanban", "calendar", "gallery"],
    usedBy: ["notion-page-clone-os"],
    agentRecipe: "Run `npx rr add notion-shell`. Zero peer deps. Import wrappers: `import { NotionPage, NotionSidebar, NotionBlock, NotionDatabase, InsertBlockButton, ViewTabs, type Block, type Database } from \"@/features/notion-shell\"`. NotionBlock ships slash menu + decorator + actions menu (wire onTurnInto/onDuplicate/onRemove). NotionDatabase ships 6 views via VIEW_REGISTRY (wire onViewActivate/onViewAdd/onViewRemove/onViewConfigChange). Property cells: text/number/checkbox/select/multi-select/status/date/url/email/phone all built in. For rich icon UX wire `renderIcon` + `renderIconPicker` to `@/features/icon-picker`.",
