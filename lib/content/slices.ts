...
// Any tab on the same origin sees the change instantly.`,
  ...
  {
    slug: "cover",
    title: "Cover — page cover image picker (gallery · upload · link · Unsplash · reposition)",
    category: "ui",
    kind: "ui",
    version: "0.1.0",
    maturity: "beta",
    tagline: "Full Notion-style cover picker: gallery (colours/gradients/textures), upload, paste URL, Unsplash search, drag-to-reposition. Props-driven, no backend lock-in.",
    description: "The complete Notion-style page cover feature, lifted from notion-page-clone and made portable. CoverBanner renders the cover band (colour / gradient / texture / uploaded / linked / Unsplash image) with hover controls — Change (opens the picker), Reposition (drag the vertical focal point), Remove. AddCoverButton is the no-cover affordance. CoverPicker is the 4-tab chooser: Gallery (12 colours + 8 gradients + Notion textures), Upload (drag/click, ≤8MB), Link (paste any https image URL), Unsplash (bundled curated landscapes + live search). The slice imports NO other slice and NO backend — the upload backend and Unsplash search are INJECTED as props (onUpload + searchUnsplash), so it drops into any app: wire onUpload to the `files` slice and searchUnsplash to a server route via unsplashSearchVia('/api/unsplash') that holds UNSPLASH_ACCESS_KEY server-side (never NEXT_PUBLIC). Ships a curated Unsplash fallback + gallery so it works with zero config. Cover values are CoverData ({ type, value, positionY?, metadata? }); parseCover normalizes legacy raw-string covers; coverStyle builds the focal-point CSS. Wired into the notion-page-clone template via NotionPage's coverSlot.",
    source: "notion-page-clone",
    slicePath: "frontend/slices/cover",
    convexPaths: [],
    npm: [],
  ...
  },
  {
    ...
    version: "0.23.0",
    maturity: "beta",
    tagline: "Portable Notion-style PAGE editor: blocks + slash menu + inline toolbar + colour + layout + built-in code/equation + cover slot. Pair with notion-sidebar + notion-database + cover.",
