...
export default convexAuthNextjsMiddleware();`,
  ...
  },
  {
    ...
    kind: "full",
    version: "0.3.0",
    tagline: "Members + invites + roles admin — tabbed, permission-gated. Props-driven, RBAC-agnostic.",
    description: "Full members + invites + roles-admin surface ported from superspace, props-driven + RBAC-agnostic. <UserManagementPanel> tabs Members + Roles: member table (search / filter / sort, inline role dropdown, soft-remove), InviteDialog + PendingInvites (resend / cancel), and a RolesPanel to create / edit / delete custom roles via a permission matrix (system roles read-only). All permission-gated. You pass `roles` + `currentPerms` + the permission catalog (resolved from rbac-roles at the app level) + CRUD/invite/role callbacks; the slice imports no other slice's frontend. Convex template ships um_members + um_invites + member & invite endpoints (gated via rbac-roles' requirePermission); roles CRUD reuses rbac-roles' listRoles / upsertRole / removeRole. P1–P3 of the user-management epic — full-parity extras (P4) next.",
    source: "superspace",
