...
export default convexAuthNextjsMiddleware();`,
  ...
  },
  {
    ...
    kind: "full",
    version: "0.5.0",
    tagline: "Members + invites + roles + teams + hierarchy — tabbed, permission-gated. Props-driven, RBAC-agnostic.",
    description: "Full members + invites + roles-admin + teams + tenant-hierarchy surface ported from superspace, props-driven + RBAC-agnostic. <UserManagementPanel> tabs Members + Roles + Teams: member table (search / filter / sort, inline role dropdown, soft-remove), InviteDialog (with an optional 'propagate to sub-workspaces' toggle — same / step-down role strategy) + PendingInvites, a RolesPanel (custom roles via permission matrix; system roles read-only), and a TeamsPanel (named user groups). All permission-gated. You pass `roles` + `currentPerms` + the permission catalog (resolved from rbac-roles) + callbacks; the slice imports no other slice's frontend. Convex ships um_members + um_invites + um_teams + um_team_members + um_tenant_links + member / invite / team / hierarchy endpoints (gated via rbac-roles' requirePermission); roles CRUD reuses rbac-roles'. The hierarchy is a generic edge tree — rr never owns the tenant entities. P1–P4b of the user-management epic — cross-workspace access matrix (P4c) next.",
    source: "superspace",
