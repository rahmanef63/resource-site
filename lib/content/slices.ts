...
export default convexAuthNextjsMiddleware();`,
  ...
  {
    slug: "database-io",
    title: "Database IO — combined CSV + JSON import/export with dynamic template",
    category: "ui",
    kind: "ui",
    version: "0.1.0",
    tagline: "One dropdown: Export CSV/JSON · Import CSV/JSON · Download dynamic template — all from live db.properties.",
    description: "Single combined import/export surface for notion-database. DatabaseIOActions ships one dropdown trigger with six items: Export CSV, Export JSON, Import CSV…, Import JSON…, Download CSV template, Download JSON template. Templates are generated dynamically from the live db.properties — header row (CSV) or wire-format v1 schema (JSON) reflects the current column set, with one placeholder row that hints at expected format per property type (e.g. `2026-01-01` for date, `Option1; Option2` for multi_select, `(computed — ignored on import)` for formula/created_time/etc.). CsvImportDialog parses + auto-maps headers (re-pickable: existing property / Title / skip / + New Property of 12 supported types, with select/status/multi_select auto-seeding options from CSV values). JsonImportDialog parses a previously-exported v1 file, diffs schema vs existing db.properties (name + type match), shows preview, emits same result shape so a single onImport handler serves both formats. Pure / props-driven. Replaces standalone database-csv + database-json (both removed CK-1E, 2026-05-22).",
    source: "notion-page-clone",
    ...
    env: [],
    peers: [
      { slug: "notion-database", range: "^0.3", reason: "Reuses Database / Page / Property / PropertyValue / PropertyType / SelectOption / DatabaseViewConfig types." },
    ],
    ...
    usedBy: [],
    agentRecipe: "Run `npx rr add database-io` (cascades notion-database as peer). Import: `import { DatabaseIOActions } from \"@/features/database-io\"`. Mount in your DB toolbar: `<DatabaseIOActions db={db} rows={rows} onImport={async ({ newProperties, rows }) => { for (const p of newProperties) await dispatchAddProperty(p); for (const r of rows) { const newId = await dispatchAddRow(); await dispatchSetTitle(newId, r.title); for (const [propId, v] of Object.entries(r.rowProps)) await dispatchSetValue(newId, propId, v); } }} />`. Single handler works for both CSV + JSON imports. Templates are generated live from db.properties — users edit the placeholder row + re-import. For lower-level access: `buildCsvTemplate(db)` / `buildJsonTemplate(db)` return strings.",
    previewPath: "/preview/slices/database-io",
    defaultView: "desktop",
    defaultZoom: 1,
  },
  {
