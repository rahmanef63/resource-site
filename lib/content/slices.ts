...
export default convexAuthNextjsMiddleware();`,
  ...
  },
  {
    ...
    compat: {
      ...
      enhances: ["doku-payment", "ai-chat", "ai-admin"],
    },
  },
  {
    slug: "ai-chat",
    ...
    kind: "full",
    version: "0.1.0",
    description: "Three-column AI chatbot — threads + attachments + tools on the left, streaming messages in the center, model picker + params + skills + actions on the right. Multi-provider (Anthropic / OpenAI / Google / Mistral / Ollama). Multimodal (text + image + PDF + audio), typed tool calls with inline inspector, agent mode (plan→execute→reflect), branching threads, prompt library, retrieval citations, usage telemetry. Convex-backed conversation store with resumable streams.",
    source: "rahmanef63/resource-site",
    docsUrl: "https://sdk.vercel.ai/docs",
    ...
    peers: [
      ...
      { slug: "ai-router", range: "^0.1", reason: "Optional — route to tiered provider proxy." },
      { slug: "vector-search", range: "^0.1", reason: "Optional — power the RAG tool with workspace embeddings." },
    ],
    tags: ["ai", "chatbot", "streaming", "multimodal", "tools", "agent", "rag", "workbench"],
    usedBy: [],
    agentRecipe: "Run `npx rr add ai-chat`. Mount `<ChatWorkbench />` at /chat. Wire `api.features.aiChat.*` queries/mutations. Streaming via HTTP action SSE; tool calls registered through ai-admin.tools registry; multimodal attachments via Convex storage. Pair with ai-admin for operator config + ai-router for tiered provider routing.",
  ...
  {
    slug: "ai-admin",
    title: "AI Admin — Config Console",
    category: "ai",
    kind: "full",
    version: "0.1.0",
    description: "Operator console for the whole AI stack — API sources (Anthropic/OpenAI/Google/Mistral/Ollama), model registry with capabilities + pricing, skill library (system prompts + tool defaults), tool registry (typed JSON schemas + sandbox flags), agent definitions (skill + model + tool subset + max-iter), per-workspace cost budgets, full audit log of every call. Plugs into the existing admin-panel section registry — no new layout needed.",
    source: "rahmanef63/resource-site",
    install: "",
    ...
    tags: ["ai", "admin", "config", "providers", "models", "skills", "tools", "agents", "budgets"],
    usedBy: [],
    agentRecipe: "Run `npx rr add ai-admin`. Adds an `AI` section to the admin-panel ADMIN_SECTIONS registry with sub-tabs (Providers/Models/Skills/Tools/Agents/Budgets/Audit). API keys encrypted at rest via AI_ADMIN_ENCRYPTION_KEY. Skills + tools + agents are SSOTs consumed by ai-chat workbench at runtime.",
    ...
    compat: {
      ...
      enhances: ["ai-chat", "ai-router", "audit-log"],
    },
  },
  {
    slug: "vector-search",
