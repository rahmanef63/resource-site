...
// Any tab on the same origin sees the change instantly.`,
  ...
  {
    slug: "md-reader",
    title: "Markdown Reader — rich-text page container",
    category: "content",
    kind: "ui",
    version: "0.1.0",
    tagline: "Read-only markdown document renderer. Same grammar as the notion editor — blocksToMarkdown(notion) renders here 1:1.",
    description: "Sibling read surface to the notion block editor. Feed <MarkdownReader content={md}/> a markdown string and it renders a clean document with rich text — headings, lists, todo, quote, GitHub-style callouts, fenced code, KaTeX equations, tables, images, <details> toggles, and inline **bold**/_em_/~~strike~~/`code`/$math$/links. Self-contained: ships its own parser (parseMarkdown → MdNode[]) + inline renderer, no notion runtime dependency. Sync is by shared grammar: the notion cluster's blocksToMarkdown / markdownToBlocks bridge (@notion/shared/lib/markdown) emits/consumes the exact same markdown this reader parses, so anything readable in the notion block page is readable here and vice-versa. Pure presentational — no store, no Convex.",
    source: "rahman-resources",
    slicePath: "frontend/slices/md-reader",
    convexPaths: [],
    npm: ["katex@^0.16", "lucide-react@^0.400.0"],
    shadcn: [],
    env: [],
    peers: [],
    tags: ["content", "markdown", "reader", "rich-text", "docs", "notion-sync"],
    usedBy: [],
    agentRecipe: "Run `npx rr add md-reader`. Render `<MarkdownReader content={markdownString} title={...} />`. To bridge from the notion editor, install the notion slice and call `blocksToMarkdown(page.blocks)` from `@notion/shared/lib/markdown`, then pass the result as `content`. For the reverse (markdown → notion blocks) use `markdownToBlocks(md)`.",
    previewPath: "/preview/slices/md-reader",
    defaultView: "desktop",
    defaultZoom: 1,
  },
  {
