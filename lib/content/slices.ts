...
export const slices: SliceEntry[] = [
  {
    ...
    kind: "backend",
    version: "0.2.0",
    tagline: "@convex-dev/auth: Password + Anonymous + Google + Resend magic-link. No Clerk.",
...
export default convexAuthNextjsMiddleware();`,
  ...
  },
  {
    ...
    kind: "ui",
    version: "0.2.0",
    tagline: "Notion-style emoji + lucide picker. Search, 10-color tint, Twemoji/native toggle.",
  ...
  {
    slug: "theme-presets",
    title: "tweakcn Theme Loader (30+ presets)",
    category: "ui",
    kind: "ui",
    version: "0.1.0",
    tagline: "30+ tweakcn presets. Click to swap CSS variables live. No backend.",
    description: "Tweakcn theme preset loader — fetches from /r/registry.json + applies CSS variable overrides to next-themes. Ships <ThemePicker> popover, <TweakcnSwitcher> for quick toggle, <ThemeColorSync> for live preview, and a 30+ preset catalog. localStorage key `nosion:theme-preset` (preserved verbatim — host can rename for new deploys). Pure React + Tailwind v4 + next-themes. No backend.",
    source: "notion-page-clone",
    ...
    env: [],
    peers: [],
    tags: ["ui", "theme", "tweakcn", "color", "preset", "notion-like"],
    usedBy: [],
    agentRecipe: "Run `npx rr add theme-presets`. Wrap dashboard with <ThemeColorSync> for live preview; drop <ThemePicker> in settings; use <TweakcnSwitcher> for quick toggle in header. Presets ship a 30+ catalog — call `applyTweakcnPreset(id)` to swap CSS variables at runtime. Storage key `nosion:theme-preset` preserved as legacy; rename via the host wrapper for new deploys.",
  ...
  },
  {
    ...
    kind: "ui",
    version: "0.1.0",
    tagline: "Notion/Excel-style drag-fill + multi-select for any grid UI.",
  ...
  },
  {
    ...
    npm: ["recharts@^2.13.0"],
    shadcn: ["button", "input", "checkbox", "dropdown-menu", "popover", "select"],
    env: [],
    peers: [
      { slug: "notion-shell", range: "^0.4", reason: "Domain types (Database, Property, PropertyValue, DbView, etc.) live in notion-shell." },
    ],
    tags: ["ui", "notion", "notion-like", "database", "table", "board", "list", "gallery", "calendar", "feed", "chart", "dashboard", "form", "map", "timeline", "gantt", "kanban", "views", "filter", "sort", "property", "files", "person", "formula", "timestamp", "unique-id", "primitive", "optional", "embeddable"],
    usedBy: ["notion-page-clone-os"],
    agentRecipe: "Run `npx rr add notion-database` (cascades notion-shell as peer). Import: `import { NotionDatabase, NotionProperty, VIEW_REGISTRY } from \"@/features/notion-database\"`. NotionDatabase ships full props-driven DB UI — host owns rows + dispatches CRUD callbacks (db.* / db.row.* / db.view.* shape from the notion-page-clone-os template store). For just one view in isolation, import the specific view component (TableView / BoardView / etc.). Property cells auto-render via renderPropertyCell helper. Pure helpers: applyView (filter + sort + search), groupBy (board kanban), bucketByDate (calendar grid). For backend persistence, copy template-base/database-silong/convex/ from the open-silong repo: handlers go to convex/, schema fragment merges into convex/schema.ts. Pick _shared/minimal/ (single-user, noop authz) or _shared/full/ (requires @convex-dev/auth + workspaces tables). See CONVEX-BACKEND.md.",
  ...
  {
    slug: "database-json",
    title: "Database JSON — import + export for notion-database",
    category: "ui",
    kind: "ui",
    version: "0.1.0",
    tagline: "JSON import + export popover for notion-database. Single onImport callback — same shape as CsvImportResult.",
    description: "JSON import + export companion for notion-database. JsonActions ships a dropdown trigger with Export (downloads `<dbname>.json` via Blob URL, wire format v1: schema + rows) + Import items. JsonImportDialog parses a previously-exported `.json` file, diffs incoming properties against existing db.properties (name + type match), shows a preview (DB icon + name + property/view/row counts + first 12 property types), then emits a single `onImport({newProperties, rows, importedDb})` callback so host writes through its own adapter (no useStore coupling). Result shape MIRRORS database-csv's CsvImportResult — a single host handler can serve both formats. Schema diff: properties matched by case-insensitive name + exact type stay mapped to existing ids; mismatched/missing properties listed as new. Pure / props-driven. Lifted from notion-page-clone, simplified for the standalone slice (dropped AI assist + cover + blocks + sub-items + templates wire fields).",
    source: "notion-page-clone",
    slicePath: "frontend/slices/database-json",
    convexPaths: [],
    npm: [],
    shadcn: ["button", "dialog", "dropdown-menu"],
    env: [],
    peers: [
      { slug: "notion-database", range: "^0.4", reason: "Reuses Database / Page / Property / PropertyValue / PropertyType / SelectOption / DatabaseViewConfig types." },
    ],
    tags: ["ui", "notion", "notion-like", "database", "json", "import", "export", "data", "backup", "wire-format", "primitive", "optional"],
    usedBy: [],
    agentRecipe: "Run `npx rr add database-json` (cascades notion-database as peer). Import: `import { JsonActions } from \"@/features/database-json\"`. Mount in your DB toolbar: `<JsonActions db={db} rows={rows} onImport={async ({ newProperties, rows, importedDb }) => { /* same handler shape as CsvActions onImport */ }} />`. Pair with CsvActions for both formats — single onImport handler works for both since result shape is identical. For lower-level use, import `exportDatabase` (DB+rows → JSON string), `parseExport` (string → DatabaseExportV1), `diffSchema` + `buildImportResult` (DatabaseExportV1 + existing properties → JsonImportResult).",
    previewPath: "/preview/slices/database-json",
    defaultView: "desktop",
    defaultZoom: 1,
  },
  {
    slug: "database-csv",
    title: "Database CSV — import + export for notion-database",
    category: "ui",
    kind: "ui",
    version: "0.1.0",
    tagline: "CSV import + export popover for notion-database. Single onImport callback — host owns persistence.",
    description: "CSV import + export companion for notion-database. CsvActions ships a dropdown trigger with Export (downloads `<dbname>.csv` via Blob URL) + Import items. CsvImportDialog parses a .csv file, auto-maps headers to existing properties (case-insensitive name match), lets the user re-pick mapping (existing property / Title / skip / + New Property of 12 supported types incl. files / person / select / multi_select / status that auto-seed options from CSV values), then emits a single `onImport({newProperties, rows})` callback so the host writes through its own adapter (no useStore coupling, no hidden mutations). Computed-by-system types (formula / created_time / last_edited_time / unique_id) are recognised + never written from CSV. Pure / props-driven. Lifted from notion-page-clone, simplified for the standalone slice (dropped relation cross-DB lookups + created_by / last_edited_by / rollup branches).",
    source: "notion-page-clone",
    slicePath: "frontend/slices/database-csv",
    convexPaths: [],
    npm: [],
    shadcn: ["button", "dialog", "dropdown-menu"],
    env: [],
    peers: [
      { slug: "notion-database", range: "^0.3", reason: "Reuses Database / Property / PropertyValue / PropertyType / SelectOption domain types." },
    ],
    tags: ["ui", "notion", "notion-like", "database", "csv", "import", "export", "data", "spreadsheet", "primitive", "optional"],
    usedBy: [],
    agentRecipe: "Run `npx rr add database-csv` (cascades notion-database as peer). Import: `import { CsvActions } from \"@/features/database-csv\"`. Mount in your DB toolbar: `<CsvActions db={db} rows={rows} onImport={async ({ newProperties, rows }) => { for (const p of newProperties) await dispatchAddProperty(p); for (const r of rows) { const newId = await dispatchAddRow(); await dispatchSetTitle(newId, r.title); for (const [propId, v] of Object.entries(r.rowProps)) await dispatchSetValue(newId, propId, v); } }} />`. Or use CsvImportDialog directly if you want your own trigger UI.",
    previewPath: "/preview/slices/database-csv",
    defaultView: "desktop",
    defaultZoom: 1,
  },
  {
