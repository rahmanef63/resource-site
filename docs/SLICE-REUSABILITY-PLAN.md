...
| **P3** 🟡 | `...props` passthrough (React 19 = ref-as-prop, no `forwardRef` needed) — **code shipped** on every host-facing interactive leaf: `files/FileUploadButton`, `full-width-toggle/FullWidthToggle` (button branches), and the two notion-shell host-facing triggers `InsertBlockButton` + `PageActionsMenu` (documented "used by hosts" / "slots via `actions` prop" — they're the *exposed boundary*, not deep internals, so passthrough is correct, not the §3 anti-pattern). **Skipped:** `library/UpvotePanel` = concurrent WIP, untouchable; truly-internal subsystem helpers (block renderers, cell editors) stay closed — consumer rewires those. | med | code via degit; ver bump + manifest **deferred** |
