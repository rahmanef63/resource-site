...
**Import rules (flatten-safe):** inside a variant, file→file imports must be relative-within-variant; variant→shared must use `@/features/<slug>/shared/...`; **never** import `@/features/<slug>/variants/<id>/...`. Slice-level renderer deps and version stay shared. A variant may declare framework-neutral `items[].deps` (`npm`, `env`, `peers`, `sharedFiles`); a single-variant install receives only that variant's runtime deps, while add-all receives their union. Renderer-specific shadcn remains at slice/framework level. `items[].convex` continues to gate provider-specific backend roots/files.
