...
/** Minimal interactive preview: CSV import + export against an in-memory
 *  notion-database. Click "CSV" dropdown → Export downloads tasks.csv,
 *  Import opens a file picker + column mapper; submitted result is
 *  persisted to component state for the session (refresh clears). */

import * as React from "react";
import { CsvActions } from "@/features/database-csv";
...
const INITIAL_ROWS: Page[] = [
  ...
  makeRow("t3", "Lift database-csv slice",   { status: "doing", priority: "p0", due: { date: "2026-05-22" } }),
];
  ...
  return (
    <main className="mx-auto min-h-screen max-w-5xl bg-background p-6">
      <header className="mb-4 flex items-center justify-between">
        <div>
          <p className="text-xs uppercase tracking-wide text-muted-foreground">database-csv</p>
          <h1 className="text-2xl font-semibold">Import + export</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Click <span className="font-mono">CSV</span> → Export to download the table.
            Import opens a column mapper that emits a single <span className="font-mono">onImport</span> callback.
          </p>
        </div>
        <CsvActions
          db={db}
