// pages-cms — block-composed multi-page CMS types.
//
// A page is a slug + metadata + an ordered list of `PageBlock`s. The
// renderer + editor switch on `block.kind`. 11 block kinds ship by
// default; add your own by extending the union + emptyBlock + the
// renderer/editor switches.
//
// Self-contained: NO cross-slice coupling. blocks[] is the only
// composition primitive (the old landing/section bridge was severed when
// this engine was promoted to a standalone slice).
...
export type PageEntry = {
  ...
  /** System pages = read-only originals (e.g. JSX-rendered routes a
   *  consumer keeps). Listed read-only in admin; can be duplicated to a
   *  custom page, never deleted/edited via the editor. */
  systemPage?: boolean;
  /** Exactly ONE page may be marked the landing page. The admin "Landing"
   *  badge surfaces it; routing is the consumer's concern. */
  isLanding?: boolean;
};
...
export type PagesAction =
  ...
  /** Replace the whole list — used by store adapters to hydrate from a
   *  backend / localStorage snapshot. */
  | { type: "PAGE_REPLACE_ALL"; payload: PageEntry[] };
