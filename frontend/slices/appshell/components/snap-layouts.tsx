...
// caption button (window.tsx owns the hover open + the mouse-leave close). Each
// LAYOUT below is a small diagram showing every zone it occupies AT ONCE (the
// real Win11 picker), not a single-zone list. Clicking a zone inside a diagram
// snaps THIS window via the existing snapWindow() store action; every rect is
// derived from snapRect() through zoneFraction(), so the flyout can never drift
// from the real tiling math (no duplicated geometry).

// Six layouts, each fully composed of REAL existing SnapZones so every zone in
// every diagram stays clickable and tiles with zero dead space. The zone
// repertoire (store-geometry.ts) has no standalone "middle third", so a true
// equal 3-column / wide-center-3-up isn't expressible without inventing new
// snap math (out of scope here) — these six instead cover every combination
// that already tiles end-to-end: halves, a 70/30 split (+ its mirror),
// quarters, and a big pane + two stacked (+ its mirror).
...
// snapRect("top") IS the full work area, so every diagram normalises against
// the SAME frame the real snap uses. snapRect is covered by
// store-geometry.test.ts, leaving this as pure division on already-tested
// geometry.
        ...
        >
          {zones.map((zone) => (
            <Button
              ...
              className="absolute h-auto min-h-0 w-auto min-w-0 rounded-[4px] bg-muted-foreground/40 p-0 hover:bg-[var(--os-accent)]"
            />
          ))}
        </div>
      ))}
