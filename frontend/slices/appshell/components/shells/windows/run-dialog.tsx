...
export function RunDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <>
      ...
      {/* Solid bg-card on purpose (NOT .material-menu / no backdrop-blur): the
          real Win+R box is a small OPAQUE dialog, not acrylic. Anchored at the
          bottom-left corner (12px above the taskbar, 12px from the left edge)
          — that corner docking is Win+R's signature, not a centered modal. */}
      <form
        onSubmit={submit}
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        {/* Authentic three-button row: OK / Cancel / Browse… (Browse has no
            real filesystem picker behind it here, so it stays disabled). */}
