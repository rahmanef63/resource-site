...
STEP 2 — shadcn + npm deps. Add any missing shadcn primitives: `npx shadcn@latest add button scroll-area dialog dropdown-menu`. npm: @phosphor-icons/react class-variance-authority clsx tailwind-merge.
...
• apps: AppDescriptor[] — { id, title, icon (a @phosphor-icons/react icon component), gradient (a CSS gradient string for the glossy icon), load: async () => ({ default: YourAppComponent }), slug?, defaultSize?: {w,h}, multi?: true (spawn a new window per open, e.g. a file manager), noDock?: true }. Your app component receives props { payload }.
