...
export const workspaces = defineTable({
  ...
  // === Display prefix ===
  // Short tag rendered before the workspace name + menu items, e.g. "FNB" → "FNB Rocket Chicken".
  // Useful for franchise / multi-brand workspaces where the same UI serves multiple tenants.
  namePrefix: v.optional(v.string()),
  // Toggle — when false the prefix is stored but not rendered. Default true if prefix set.
  namePrefixEnabled: v.optional(v.boolean()),
  ...
  // === Registry Metadata ===
  // Additive workspace-launch metadata for desktop/mobile surfaces.
  registry: v.optional(
    v.object({
      mobileLabel: v.optional(v.string()),
      desktopLabel: v.optional(v.string()),
      workspaceCategory: v.optional(
        v.union(
          v.literal("core"),
          v.literal("business-app"),
          v.literal("tool"),
          v.literal("external-link"),
        ),
      ),
      launchType: v.optional(
        v.union(
          v.literal("internal"),
          v.literal("external"),
          v.literal("rendered"),
      ...
      mobileRoute: v.optional(v.string()),
      sourceManifestUrl: v.optional(v.string()),
      blockManifestUrl: v.optional(v.string()),
      ...
      isLauncherVisible: v.optional(v.boolean()),
    }),
  ),
