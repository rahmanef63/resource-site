...
//
// SCAFFOLDED NON-FUNCTIONAL: rr wired this FAB to a shared-agentic model seam
// (runAgentLoop + global tool registry). superspace has no such seam, so the
// agent path is removed and the widget always answers with
// `stubReply` — it ships inert and never needs a key at build/prerender time.
// Re-wire to `api.features.aiRouter.action.callModel` in a supervised follow-up.
//
// Usage in your app/layout.tsx:
//   import { ChatFab } from "@/frontend/slices/ai-router/components/chat-fab";
//   <ChatFab tier="mid" />
...
export function ChatFab({
  ...
  async function send() {
    ...
    try {
      // Non-functional scaffold: always answers with the local stub. Wire a
      // real backend (api.features.aiRouter.action.callModel) to enable replies.
...
async function stubReply(prompt: string): Promise<string> {
  ...
  return `(stub) Received: "${prompt.slice(0, 80)}${prompt.length > 80 ? "…" : ""}". Wire ChatFab to api.features.aiRouter.action.callModel for real responses.`;
}
