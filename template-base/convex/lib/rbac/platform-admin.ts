...
// ============================================================================
// CONVEX CTX HELPERS — @convex-dev/auth aware
// ============================================================================
...
/**
 * Get current user from a Convex query/mutation context (via
 * @convex-dev/auth). Returns null if unauthenticated.
 */
...
/**
 * True if the authenticated user's email is listed in
...
export async function isCurrentUserPlatformAdmin(
  ...
  return isPlatformAdmin(user?.email ?? null);
}

/**
 * Throws if not a platform admin. Use at the top of platform-level
 * mutations/queries.
 */
export async function requirePlatformAdmin(
  ctx: MutationCtx | QueryCtx,
): Promise<void> {
  const ok = await isCurrentUserPlatformAdmin(ctx);
  if (!ok) {
    throw new Error(
      "Platform admin required. Set PLATFORM_ADMIN_EMAILS via `npx convex env set`.",
    );
  }
}
