...
|------|------|---------|-------------|
| `preset` | `"feature" \| "store" \| "admin" \| "ide"` or config | - | Preset width + responsive defaults (can be overridden). |
| `left` | ReactNode | optional | Left panel content (auto-hidden when `null/undefined`) |
| `center` | ReactNode | required | Center panel content |
| `right` | ReactNode | optional | Right panel content (auto-hidden when `null/undefined`) |
