...
function FilterValueSelector<T = unknown>({ field, values, onChange, operator }: FilterValueSelectorProps<T>) {
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2">
          <Switch checked={isChecked} onCheckedChange={(checked) => onChange([checked as T])} className="scale-90" />
    ...
    return (
      ...
      >
        <SharedDatePicker
          ...
          onChange={(d) =>
            onChange([d ? format(d, 'yyyy-MM-dd') : '', endDate] as T[])
          }
          placeholder="From"
          className={cn('w-36', field.className)}
        />
        ...
        <SharedDatePicker
          ...
          onChange={(d) =>
            onChange([startDate, d ? format(d, 'yyyy-MM-dd') : ''] as T[])
          }
          placeholder="To"
          className={cn('w-36', field.className)}
        />
  ...
  if (field.type === 'date') {
    ...
    return (
      <SharedDatePicker
        date={raw ? parseISO(raw) : undefined}
        onChange={(d) => onChange([d ? format(d, 'yyyy-MM-dd') : ''] as T[])}
