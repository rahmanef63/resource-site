...
const CONFIG_GLOBS = ["frontend/slices/*/config.ts", "frontend/slices/*/config/index.ts"]
...
async function loadFeatureEntries(): Promise<FeatureEntry[]> {
  const configPaths = globSync(CONFIG_GLOBS, {
    cwd: ROOT_DIR,
    windowsPathsNoEscape: true,
  })

  if (configPaths.length === 0) {
    throw new Error("No feature config files found under frontend/slices/*")
  }

  const entries: FeatureEntry[] = []
  const seenSlugs = new Set<string>()

  // Sort paths so config.ts comes before config/index.ts for the same feature
  configPaths.sort((a, b) => a.length - b.length)

  for (const relativePath of configPaths) {
    const normalizedPath = relativePath.replace(/\\/g, "/")
    const isNested = normalizedPath.endsWith('config/index.ts')
    const slug = path.basename(isNested ? path.dirname(path.dirname(normalizedPath)) : path.dirname(normalizedPath))
